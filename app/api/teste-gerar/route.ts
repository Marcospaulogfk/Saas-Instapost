...
export async function POST(req: Request) {
  ...
  claudeResult.data.slides.forEach((s, i) => {
    ...
    console.log(`  ✨ highlight_words:  ${JSON.stringify(s.highlight_words)}`)
    if (s.subtitle) console.log(`  📝 subtitle:         ${JSON.stringify(s.subtitle)}`)
    if (s.body) console.log(`  📄 body:             ${JSON.stringify(s.body)}`)
    if (s.cta_badge) console.log(`  🎯 cta_badge:        ${JSON.stringify(s.cta_badge)}`)
