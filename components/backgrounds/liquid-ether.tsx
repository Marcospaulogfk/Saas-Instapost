/* Vendored do React Bits (Backgrounds/LiquidEther, versão ts-tailwind).
 *
 * MODIFICAÇÃO LOCAL: a prop `interactive` foi acrescentada — o original não
 * tem. Com `interactive={false}` o componente não registra nenhum listener de
 * mouse/touch, então o fluido roda só pelo autoDemo e ignora o cursor. Sem
 * isso, um fundo que ocupa a tela inteira deixa o ponteiro "dentro" do
 * container quase o tempo todo, e o AutoDriver para (ele desliga sempre que
 * `isHoverInside` é true) — na prática o autoDemo nunca rodaria.
 *
 * Ao atualizar o componente pela fonte, reaplicar essa mudança: procure por
 * `interactive` (interface, destructuring, MouseClass.init e a chamada do
 * WebGLManager).
 */
...
export interface LiquidEtherProps {
  /** false = ignora o cursor por completo; o fluido só se move pelo autoDemo. */
  interactive?: boolean;
  mouseForce?: number;
  cursorSize?: number;
  isViscous?: boolean;
...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    class MouseClass {
      ...
      init(container: HTMLElement, interactive = true) {
        this.container = container;
        this.docTarget = container.ownerDocument || null;
        // Sem interação: guarda o container (o AutoDriver precisa dele pra
        // normalizar coordenadas) e sai antes de assinar qualquer listener.
...
`;
    ...
    class WebGLManager implements LiquidEtherWebGL {
      ...
      constructor(props: any) {
        ...
        Mouse.init(props.$wrapper, props.interactive !== false);
