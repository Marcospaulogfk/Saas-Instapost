...
**REGRA GLOBAL — template como ÁGUA (widget nunca atrapalha widget).** TEXTO é SÓLIDO: zona `flex-shrink-0`, título e corpo com `line-clamp-N` (corte em limite de LINHA, nunca em pixel), título com tamanho adaptativo. IMAGEM é ÁGUA: zona `flex-1 min-h-0` SEM aspect-ratio fixo (`min-h-[96px]` de segurança), `<img>` absoluta object-cover — preenche exatamente o que sobra. ⚠️ Causa raiz do bug histórico: `line-clamp`/`overflow:hidden` zera a min-height do flex item e o flexbox corta o TEXTO em pixel enquanto a imagem de aspect fixo não cede — NUNCA usar "texto flex-1 overflow-hidden + imagem aspect fixo". Aplicado nos 8 splits + legado (`editorial-splits.tsx`, `slide-preview.tsx`, commit `1f6cbb8`). Testar sempre com textos longos nos 8 estilos.

**REGRA GLOBAL — enquadramento da foto (nunca corta a cabeça).** Foto de CONTEÚDO em caixa (Revista/Gradiente/Minimal/MyPostFlow/Bolo…) usa `object-cover`, cujo `object-position: center` padrão corta o TOPO quando a caixa é mais larga/baixa que a foto — e retratos têm o rosto no terço superior, então a cabeça some. Regra: todo `object-cover` de foto de conteúdo usa `PHOTO_FOCUS` (`editorial-shared.tsx`, hoje `"50% 20%"` — viés pro topo) via `style={{ objectPosition }}`. Full-bleed 4:5 (Seamless/capas) NÃO usa (a caixa já é retrato, center enquadra certo) e avatares também não. Heurística (não é detecção de rosto): assume sujeito no topo. Aplicado em `editorial-splits.tsx` (SingleImageBox + inline) e `slide-preview.tsx` (LegacySlideImage), commit `297c7ce`.
...
- **Ordem de deploy (obrigatória):** migrations 0014 → 0016 → 0020 → 0021 → 0026 ANTES de qualquer redeploy. Sem a 0020 a RPC `apply_tokens` não existe, todo débito falha e a geração fica de graça. Arquivo pronto pra colar no SQL Editor: `APLICAR-EM-PRODUCAO.sql` na raiz.
- **Cadastro (01/09/2026):** pede nome (vai em `user_metadata.full_name`) e, depois de criar a conta, uma etapa skippable "como você vai usar o Nexus?" (`/comecar`, cards estilo Canva) grava `users.objetivo_uso` (migration 0026, null = pulou). `/comecar` é de propósito FORA do prefixo `/onboarding/*` — esse já é o wizard de configurar MARCA (`app/onboarding/objetivo` é outra tela, "objetivos de marketing", nada a ver com esta). Sem cartão no cadastro — o trial de 45 tokens nasce pelo trigger `handle_new_user` (0010/0020), igual antes.
- **Checkout transparente (01/09/2026):** assinatura paga por cartão dentro do app (`app/api/billing/assinar-cartao`, modal em `components/billing/`) em vez de redirecionar pro checkout hospedado — `POST /v3/subscriptions` da Asaas com `creditCard` direto, cobrança IMEDIATA (`nextDueDate` = hoje; o trial já foi dado no cadastro, então aqui não tem "grátis por N dias"). Payloads/validação em `lib/billing/asaas-cartao.ts` + `lib/cartao.ts` (Luhn/CPF/CNPJ, client-safe). A ativação otimista reusa `lib/billing/apply.ts` (o MESMO caminho do webhook) com um evento `payment_confirmed` sintético — só é disparado se a Asaas já confirmou o pagamento síncrono (`GET /payments?subscription=`); a dedupe funciona porque usa o `paymentId` REAL, então quando o webhook de verdade chegar ele cai no `already` de `onPaymentConfirmed` e não credita em dobro. Se a Asaas responder indisponibilidade (503), a UI oferece o botão de fallback pro checkout hospedado (`iniciarCheckout`, que continua existindo). Troca de plano é de graça: `onPaymentConfirmed` já cancela a assinatura anterior na Asaas por conta própria (`switchedSubscription`), sem código novo.
